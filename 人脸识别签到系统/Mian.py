...
# 注册窗口
class child_Window(QDialog, Ui_Register):

    def __init__(self, parent=None):
        super(child_Window, self).__init__(parent)
        self.setupUi(self)
...
# 缺勤窗口
class child_Window_late(QDialog, Ui_Table):
    def __init__(self, parent=None):
        super(child_Window_late, self).__init__(parent)
        self.setupUi(self)
        self.CallBackFunctions()
    ...
    # 回调函数
    def CallBackFunctions(self):
        self.ResetBt.clicked.connect(self.setTable)
...
if __name__ == '__main__':
    ...
    ui = parentWindow()
    ui_child = child_Window()
    ui_child_late = child_Window_late()
